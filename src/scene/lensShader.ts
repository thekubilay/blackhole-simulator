...
void sampleDisk(vec3 hp, vec3 vn, inout vec4 acc){
  ...
  float nA = 0.50*fbm(qA*1.35) + 0.32*fbm(qA*3.6) + 0.18*fbm(qA*7.4);
  float nB = 0.50*fbm(qB*1.35 + 31.7) + 0.32*fbm(qB*3.6 + 11.3) + 0.18*fbm(qB*7.4 + 5.9);
  float n = mix(nA, nB, smoothstep(3.2, 8.5, rr));
...
void main(){
  ...
  for(int i=0;i<260;i++){
    ...
    float dt = (0.045 + 0.065*max(r-1.6, 0.)) * (150.0/float(uSteps));
